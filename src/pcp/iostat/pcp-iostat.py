...
class IostatReport(pmcc.MetricGroupPrinter):
    ...
    def report(self, manager):
        ...
        if "h" not in IostatOptions.xflag:
            ...
            if self.Hcount == 1:
                if "t" in IostatOptions.xflag:
                    heading = ('# Timestamp', 'Device',precision+5, 'rrqm/s',precision+5, 'wrqm/s',precision+5, 'r/s',precision+4,\
                    'w/s',precision+6, 'rkB/s',precision+6, 'wkB/s', precision+6,'avgrq-sz',precision+6, 'avgqu-sz',precision+5, \
                    'await',precision+5, 'r_await', precision+5,'w_await',precision+5, '%util')
                else:
                    heading = ('# Device',precision+5, 'rrqm/s',precision+5, 'wrqm/s',precision+5, 'r/s',precision+4, 'w/s'\
                    ,precision+6, 'rkB/s',precision+6, 'wkB/s', precision+6,'avgrq-sz',precision+6, 'avgqu-sz',precision+5,\
                    'await',precision+5, 'r_await',precision+5, 'w_await',precision+5, '%util')
            ...
            for inst in sorted(instlist):
                ...
                if "t" in IostatOptions.xflag:
                    if badcounters:
                        ...
                        print(valfmt % (timestamp, device,precision+5, precision, rrqm,precision+5,precision, wrqm,precision+4,precision,\
                    ...
                    if badcounters:
                        ...
                        print(valfmt % (device,precision+5, precision, rrqm,precision+5,precision, wrqm,precision+5,precision, r,precision+4,\
